...
def close_sink(sink: socket.socket):
    sink.close()

def message_from_sink(sink: socket.socket):
    conn, addr = sink.accept()
    with conn:
...
def test_record_version(setup_sink):
    data = run_server(
        TcpProxy(
            address=NETWORK_ADDRESS,
            record_version=TlsVersion.TLS13.value
        ),
        setup_sink
    )
...
def run_server(proxy: TcpProxy, sink: socket.socket) -> bytes:
    """
